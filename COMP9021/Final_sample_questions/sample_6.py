...
def is_valid_prefix_expression(expression):
    ...
    try:
        if any(not(c.isdigit() or c.isspace() or c in '+-*/') for c in expression):
            raise ValueError
        tokens = re.compile('(\+\d+|\-\d+|\d+|\+|\-|\*|/)').findall(expression)
        
        for i in range(len(tokens) - 1):
            tmp = tokens[i] + tokens[i + 1]
            if tmp in expression:
                raise ValueError
        ...
        while ('+' in stack) or ('-' in stack) or ('*' in stack) or ('/' in stack):
            for i in range(len(stack)):
                if stack[len(stack) - 1 - i] == '+' or \
                    stack[len(stack) - 1 - i] == '-' or \
                    stack[len(stack) - 1 - i] == '*' or \
                    stack[len(stack) - 1 - i] == '/':
                    operator = stack[len(stack) - 1 - i]
                    idx = len(stack) - 1 - i                   
                    
                    arg_2 = stack[idx + 2]
                    arg_1 = stack[idx + 1]
                    if idx + 1 > len(stack) - 1 or idx + 2 > len(stack) - 1:
                        raise ValueError
                    
                    if isinstance(arg_1, str) or isinstance(arg_2, str):
                        raise ValueError
                
                    value = {'+': add, '-': sub, '*': mul, '/': truediv}[operator](arg_1, arg_2)
                    stack[idx] = value
                    if idx + 3 > len(stack) - 1:
                        stack = stack[:idx + 1]
                    else:
                        stack = stack[:idx + 1] + stack[idx + 3:]
                    break
        result = stack.pop()
...
print(evaluate_prefix_expression('12'))
print(evaluate_prefix_expression('+ 12 4'))
print(evaluate_prefix_expression('- + 12 4 10'))
print(evaluate_prefix_expression('+ - + 12 4 10 * 11 4'))
print(evaluate_prefix_expression('/ + - + 12 4 10 * 11 4 5'))
print(evaluate_prefix_expression('+ / + - + 12 4 10 * 11 4 5 - 80 82 '))
print(evaluate_prefix_expression('+ +1 2'))
print(evaluate_prefix_expression('+ +1 -2'))
