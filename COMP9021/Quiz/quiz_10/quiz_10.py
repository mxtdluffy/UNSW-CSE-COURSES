...
class BinaryTree():
    ...
    def _print_extend_tree(self, n, height):
        global tree_list
        if n > height:
            return
        if self.value is None:
            tree_list.append(['a' * (2 ** (height - n + 1) - 1)])
        else:
            self.left_node._print_extend_tree(n + 1, height)
            tree_list.append(['xxxxxx' * n, self.value])
            self.right_node._print_extend_tree(n + 1, height)

    def print_extend_tree(self):
        global tree_list
        if self.value is None:
            return 
        self._print_extend_tree(0, self.height())
        

def get_space(tree_list, idx):
    spaces = 0
    for i in range(idx):
        if len(tree_list[i]) == 1:
            spaces += len(tree_list[i][0])
        else:
            spaces += 1
    return spaces
...
def print_growing_up(tree):
    ...
    tree.print_extend_tree()
    tmp = []
    output_str = ''
    for i in range(len(tree_list)):
        if len(tree_list[i]) == 2:
            cur = tree_list[i]
            cur.append(i)
            tmp.append(cur)
    tmp = sorted(tmp)
    ...
    for i in range(len(tmp)):
        ...
        value = tmp[i][1]
        index = tmp[i][2]
        space = get_space(tree_list, index)
        mark.append(length)
        output.append(' ' * space + str(value))
    ...
    while i < len(output) - 1:
        if not mark[i] == mark[i + 1]:
            ...
            j = i + 1
            cur_out = combine(output[i], output[j])
            while mark[j] == mark[j + 1]:
                j += 1
                cur_out = combine(cur_out, output[j])
            print(cur_out)
            i = j + 1
            if i == len(output) - 1:
                print(output[i])
